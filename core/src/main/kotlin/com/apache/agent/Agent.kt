...
sealed class AgentResult {
        ...
        data class NeedsConfirmation(val confirmationId: String, val warning: String) :
                AgentResult()
}
 ...
 *     c. Si se puede ejecutar directo -> se guarda la function call del modelo,
 *        se ejecuta la tool, se guarda su resultado y se manda de vuelta a Gemini
...
class Agent(
         ...
         * recursivamente tras ejecutar una tool (Gemini necesita "ver" el resultado de la función
         * para redactar la respuesta final) y también tras confirmar una acción pendiente.
         */
        private fun runTurn(conversationId: String): AgentResult {

                val history = memoryRepository.getHistory(conversationId)

                return when (val geminiResult =
                                geminiClient.sendMessage(history, SYSTEM_INSTRUCTION)
                ) {
                        ...
                        is GeminiResult.FunctionCall ->
                                handleFunctionCall(conversationId, geminiResult)
                }
        }

        private fun handleFunctionCall(
                conversationId: String,
                call: GeminiResult.FunctionCall
        ): AgentResult {
                ...
                // Guardamos la llamada original del modelo para que GeminiClient pueda

                // reconstruir el functionCall correctamente al enviar de nuevo el historial.
                ...
                return when (permissionManager.decide(tool.riskLevel)) {
                        PermissionDecision.EXECUTE_DIRECT -> {
                                ...
                                // El resultado de la función se guarda como un turno "function"
                                // junto con
                                // el nombre de la tool, para que Gemini pueda reconstruir el
                                // functionResponse
                                // correctamente en la siguiente llamada y redactar la respuesta
                                // definitiva.
                                // También guardamos el id de la llamada para que Gemini pueda
                                // asociar correctamente el functionResponse con el functionCall.
