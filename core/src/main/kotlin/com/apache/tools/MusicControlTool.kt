 ...
 *
 * Riesgo REVERSIBLE (como [OpenApplicationTool]): las acciones tienen un efecto real (cambian lo
 * que suena o el volumen), pero son benignas y fácilmente deshacibles, así que se ejecutan
 * directamente sin pedir confirmación, quedando registradas en el log del agente.
 *
 * IMPORTANTE (diseño): esta tool NO sabe hablar con Spotify, YouTube Music ni ningún reproductor en
 * concreto. Solo conoce la interfaz [MusicSource] y delega en [MusicSourceManager] para saber a qué
 * fuente mandar cada comando. Así, cuando en la Fase 5 del roadmap se añadan integraciones reales,
 * esta clase no necesita cambiar: basta con registrar una nueva MusicSource (ver NoOpMusicSource
 * para un ejemplo mínimo).
 */
@Component
class MusicControlTool(private val sourceManager: MusicSourceManager) : Tool {
                            ...
                            mapOf(
                                    "action" to
                                            mapOf(
                                                    "type" to "string",
                                                    "description" to
                                                            "Acción a realizar sobre la reproducción actual.",
                                                    "enum" to
                                                            listOf(
                                                                    "play",
                                                                    "pause",
    ...
    override fun execute(args: Map<String, Any?>): String {
        ...
        return when (action) {
            ...
            "volumeUp" -> {
                val success = source.volumeUp()

                if (success) {
                    val volume = source.getStatus().volumePercent
            ...
            "setVolume" -> {
                ...
                val volume =
                        (args["volume"] as? Number)?.toInt()
                                ?: (args["volume"] as? String)?.toIntOrNull()
                                        ?: return "Necesito un valor entre 0 y 100."

                val clamped = volume.coerceIn(0, 100)

                resultOf(
                        source.setVolume(clamped),
                        "Volumen al $clamped%.",
                        action
                )
            }
            ...
            "status" -> describeStatus(source)
                    ...
                    "Acción '$action' no reconocida. Acciones disponibles: " +
    ...
    private fun describeStatus(source: MusicSource): String {
        ...
        if (!status.isPlaying && status.title == null) {
            return "No se está reproduciendo nada ahora mismo."
        }

        val trackText =
                listOfNotNull(
                                status.title,
                                status.artist?.let { "de $it" }
                        )
                        .joinToString(" ")
                        .ifBlank { "una canción sin título/artista disponible" }
        ...
        val volumeText =
                status.volumePercent?.let { " · volumen $it%" } ?: ""

        return "$stateText $trackText$volumeText."
    }
    ...
    private fun resultOf(
            ...
            if (success) {
                ...
                "No he podido ${describeAction(action)}."
            }
